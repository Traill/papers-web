...
trait ITA2014 extends XMLParser with PDFLoader
                                with CleanUnicode {

  // Takes a doc and parses it according to the information we have from ITA2014
  def parseDoc(doc : Document) : Option[Document] = {
    ...
    for (p <- paper; d <- setInfo(doc, p)) yield d
  }

  // Sets info of paper according to information we have from ITA2014
  private def setInfo(doc : Document, paper : Paper) : Option[Document] = {
    val path = Analyzer.filePath + doc.id
    val info : List[String] = Source.fromFile(path).getLines.toList
    if (info.length < 8) {
      //throw new Exception("Corrupted paper file: " + doc.id + ".txt")
      println("Corrupted paper file: " + doc.id + ".txt")
      return None
    }
  ...
  private def getPdf(doc : Document) : Option[Paper] = None
